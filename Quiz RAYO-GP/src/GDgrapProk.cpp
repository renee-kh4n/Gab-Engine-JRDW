...
#pragma region Object setup
    ...
    auto mLineHandler = new LineHandler();
    ...
    root_object->RegisterHandler(mLineHandler);
    ...
    auto CreateParticleFunction = [drawcalls, root_object, unlitShader, mRenderPipeline]() {
        ...
        //Sphere collider
        auto collider = new Collider(5.0f);
    ...
    //Collision testing
    auto b1 = CreateParticleFunction();
    b1->SetParent(root_object);
    b1->TranslateLocal(Vector3(150, 200, 0));
    b1->velocity = Vector3(0, 0, 0);

    auto bungee = new Bungee(0.6, 200);
    bungee->TranslateLocal(Vector3(150, 200, 0));
    bungee->to_rbody = b1;
    bungee->SetParent(root_object);

    auto b2 = CreateParticleFunction();
    b2->TranslateLocal(Vector3(-220, 200, 0));
    b2->SetParent(root_object);
    b2->velocity = Vector3(0, 0, 0);

    auto chain = new ChainJoint(200);
    chain->TranslateLocal(Vector3(-150, 200, 0));
    chain->to_rbody = b2;
